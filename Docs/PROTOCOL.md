...
│   └── hooks/
└── vite.config.js
Lead Dev Note: Prohibit max-width in media queries to ensure fluid escalation. max-width on specific elements for containment is permitted; however, layout logic must remain min-width driven.